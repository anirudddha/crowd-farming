...
import { Menu, X, ShoppingCart as LucideCart, Home, Users, Tractor, Sprout, Handshake, Store, User, Bell, ChevronDown, LogOut, LayoutDashboard, ShoppingBag } from 'lucide-react';
          ...
          <div className="flex items-center justify-between h-20">
            ...
            <Link to="/" className="flex items-center space-x-3 group" aria-label="GenHarvest Home">
              ...
              <span className="text-xl sm:text-2xl font-bold text-gray-900 font-sans">
                ...
                <span className="text-gray-600 ml-1.5">Harvest</span>
              </span>
            </Link>
            ...
            <nav className="hidden lg:flex items-center space-x-7 ml-12">
              ...
              <NavLink to="/about" current={pathname} text="About" icon="users" />
        ...
        <nav className={`absolute top-0 right-0 h-full w-full max-w-sm bg-white shadow-xl transition-transform duration-300 ${isMenuOpen ? 'transform-none' : 'translate-x-full'}`}>
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-2">
              ...
              <MobileNavLink to="/about" text="About" current={pathname} onClose={toggleMenu} Icon={Users} />
...
const AuthButton = ({ to, variant = 'primary', children, className = '', onClick }) => (
  <Link
    to={to}
    onClick={onClick}
    className={`px-5 py-2.5 rounded-lg font-medium transition-all flex items-center justify-center text-sm ${className} ${variant === 'primary' ? 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm hover:shadow-md' : 'text-gray-700 bg-gray-100 hover:bg-gray-200'
      }`}
  >
    {children}
  </Link>
